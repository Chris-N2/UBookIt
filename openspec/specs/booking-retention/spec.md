...
That a site may configure a period after which the personal data on its bookings is erased
automatically, and everything that follows from making erasure happen with nobody asking for it:
what the clock is measured from and which bookings it reaches; that the feature is off unless
configured and that a configuration which cannot be read means off rather than a guessed period;
that the sweep erases through the package's single erasure verb, completes rather than doing a
sample of its work, terminates, and is safe to repeat, interrupt and run on one server of many;
that it selects by time alone and handles no contact detail; and that what it does — including
that it cannot be undone and that switching it on erases a site's history at once — is documented.

Stated as its own capability rather than inside `booker-erasure` because it is a policy a site
adopts, not a property of the operation it uses. The operation is the same one an operator
invokes by hand; what is new here is a clock, a way to find what the clock has caught, and the
obligations that let an erasure exist with no caller to hold to account.
