...
Defines services as a first-class bookable concept: a domain `Service` with a display name, a duration specification that is either fixed or variable within optional bounds, and one or more required roles, each naming a distinct resource type and the capabilities a resource must carry to fill it; Core-factory validation with stable machine-readable failure codes; the duration semantics that govern appointment length when a service is booked; and an authorized versioned Management API surface with purpose-built DTOs that keeps raw booking storage out of the HTTP layer. Defining services is purely additive — the existing direct-resource booking path is unaffected.
## Requirements
