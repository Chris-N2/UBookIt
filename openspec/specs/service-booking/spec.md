...
## Purpose
TBD - created by archiving change book-via-service. Update Purpose after archive.
## Requirements
### Requirement: Eligible resource resolution
`UBookIt.Core` SHALL resolve a service's single role to its candidate pool: every resource whose type key equals the role's `ResourceType`, further restricted to those whose booking constraints admit at least one length permitted by the service's duration specification (`ServiceDuration.TryResolveAgainst` succeeding). Eligibility SHALL be determined by resource type key alone; no capability, tag, or explicit service-to-resource association participates in v1.
...
A resource excluded because its constraints admit no permitted length SHALL be excluded silently — that is an answer about that resource, not a validation failure of the service.

#### Scenario: Pool is every resource of the role's type
- **WHEN** a service's role names resource type `room` and four resources of type `room` and three of type `therapist` exist
