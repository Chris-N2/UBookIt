...
Who may do what within the uBookIt section, decided by three permission verbs a site
...
authorization, never data on a group), and configuring resources, services and
responsibility. The section grant stays the unremovable outer gate that no verb can
substitute for, and the Sensitive data group stays the decisive control over contact
details, joined by the read verb and replaced by nothing.

The verbs live in Umbraco's own group permission storage; the package stores only a
one-shot marker recording that existing section-granted groups were seeded with all
three at upgrade, so nobody loses access by upgrading and the toggles thereafter mean
exactly what they show — empty is nothing. The client hides what a user's verbs do not
cover as a courtesy; the server decides every request.
