...
## 9. QA round 3 remediation (REJECT, 1 MAJOR / 1 MINOR / 1 NIT)

- [x] 9.1 MAJOR: the scenario, strengthened at round 2 to "in any position or YAML style", was falsified by a quoted job name and a `\x`-escaped key (QA parsed both with js-yaml). The fix is a fifth test, `Every_workflow_is_written_in_the_plain_YAML_these_guards_read`. Outside block scalars it refuses quoted keys, backslashes, anchors, aliases and merge keys. Job headers also recognise quoted names. The spec scenario now lists exactly what is caught, says the non-plain forms are refused rather than interpreted, and names what rests on review. A probe of today's three workflows found no false positives outside block scalars before the rule went in. Evidence: QA's (a) and (b) now fail. So do a quoted but unescaped `"id-token"` key and an anchor on `permissions`. All 14 earlier mutations still fail, 21 in all, then 5/5 once restored.
- [x] 9.2 MINOR: the git/gh regex now also accepts a quote or `/` before the command word. The publish job may not reference `github.com`, `githubusercontent.com`, `codeload.github`, `github.repository` or `github.server_url`. Evidence: `run: "git clone x"`, `/usr/bin/git clone`, and `curl raw.githubusercontent.com/... | bash` in the publish job each fail. A fetch from another host is stated in the test remarks and the spec as resting on review.
- [x] 9.3 NIT: the fallback note names BaGet, and says the skip is client-side.

## 10. QA round 4 remediation (REJECT, 1 MAJOR / 1 NIT)

- [x] 10.1 MAJOR: the block-scalar exemption could be abused. After `- name: |`, a line at the step's key column was "block body" to the guard but a sibling key to YAML, so a `"uses": actions/checkout@v4` passed 5/5; QA parsed it with js-yaml. Taking QA's option (iii): quoted keys, anchors, aliases and merge keys are refused on EVERY line, and only the backslash rule stays scoped to YAML lines. Option (i) as well: the `- key: |` block indent is the key's column. `Uses()` also reads a quoted `uses` key. The spec scenario and test remarks are reworded to match what the code decides. Evidence: a probe of every line of every workflow found no false positives before the rule went in. QA's shape in the publish job fails 3 tests, the `>-` variant in ci.yml fails 2, and a block-shaped `<<: *inherited` fails. All 21 earlier mutations still fail, 24 in all, then 5/5 once restored.
- [x] 10.2 NIT (accepted, no change): the plain-YAML rule refuses `'on':` and backslashes in YAML-level values such as Windows paths or hashFiles patterns. That is the chosen trade-off; the failure message says how to write it plainly.

## 11. QA round 5 remediation (REJECT, 1 MAJOR)

- [x] 11.1 MAJOR: a `uses:` value on the next line, and the `?` explicit key, each carried an unpinned checkout past all 5 tests with no quote, escape, anchor, alias or merge key. This was the fourth round in which the text reading lost to a neighbouring YAML form. Taking QA's option (2), which ends the series: `YamlDotNet` 18.1.0 (MIT) is added to the TEST project only (`Directory.Packages.props` and `UBookIt.Tests.csproj`; no advisories per `dotnet list package --vulnerable`). Every rule is now asserted over the parsed tree: identity-token grants at any depth; `write-all`; workflow-level permissions; the publish job's permissions being exactly `id-token: write`; pinning of every `uses` value; the publish job's allow-list, git/gh and repository-URL checks over every scalar in the job; environment, `if` and `needs` compared as values; and the runbook values. Anchors (so no aliases), merge keys and a second document are refused in the tree. The round-3/4 text rules remain as a backstop. The spec scenario now lists only what is actually refused, and design D9 records the reversal of its rejected alternative. Evidence: all 29 mutations fail. That includes QA's (a) next-line value, (b) `? uses`, and (c) a flow mapping spanning lines, none of which the text backstop can see; an explicit `? id-token` key; and a second YAML document. Every earlier mutation still fails, then 5/5 once restored.

## 12. QA round 6 remediation (REJECT, 1 MAJOR / 3 NIT)

- [x] 12.1 MAJOR: the spec scenario said the suite fails when the publish step "invokes git or gh", and QA's `G=git; $G clone` (M4) and `"$(command -v git)" clone` (M5) passed. The claim is narrowed to what a text check can decide: "names git or gh as a command word". The spec's THEN clause and the test remarks both say that git or gh invoked indirectly (a command assembled by the shell) rests on review, alongside fetches from other hosts. The regex is also tightened, as QA suggested: `=` joins the prefix class, and a word boundary replaces the trailing whitespace. Evidence: M4 and M5 both fail.
