...
- [ ] 1.1 Extract from `BookingFormBuilder` the parts that are genuinely about *a
...
- [ ] 1.2 **The resource flow must not change behaviour while its guts move.** Its
...
- [ ] 1.3 Shared partials for the markup that must not drift: `_DateAndLength`,
...
- [ ] 2.1 A service flow rendering date + length → times → details → POST →
...
- [ ] 2.2 **Write it for several resources from the start.** A single-role service
...
- [ ] 2.3 Length control per design D5: `Variable` renders the control bounded by the
...
- [ ] 2.4 Anti-forgery, PRG and input-preserving failure handling on the same terms
...
- [ ] 2.5 **`pinnedResourceId` is never sent** (design D7). Not defaulted, not
      hidden-fielded: absent. Assert no code path sets it.

## 3. Refusals — the substance, not the trimming

- [ ] 3.1 Map `conflict` and `service-unavailable` to two distinct visitor-facing
      shapes, from the **stable code** and never from message text.
- [ ] 3.2 **The covering test is the pair.** A suite proving "a service books on a
...
- [ ] 3.3 **Disclosure:** assert the deterministic refusal names no role, no resource
...
- [ ] 3.4 Assert the backoffice shortfall report is unchanged by all of the above.

## 4. Catalogue and entry points

- [ ] 4.1 Catalogue listing services and `DirectlyBookable` resources together,
      composed from the existing stores. No new stored notion of what is bookable.
- [ ] 4.2 A resource withholding direct booking does not appear. The covering test
      needs a withholding resource *and* a permitting one, or it cannot fail.
- [ ] 4.3 The dispatcher of design D2: no id → catalogue; `serviceId` → service flow;
      `resourceId` → resource flow; an explicitly supplied id beats the query.
- [ ] 4.4 Assert the existing `Booking` component still works when invoked directly,
      unchanged — someone may already have it in a template.
- [ ] 4.5 Assert reaching a service flow directly and by way of the catalogue produce
      the same steps and controls from that point on.

## 5. State in the URL

- [ ] 5.1 Service/resource, date and length in the query string; each step linkable
      and bookmarkable.
- [ ] 5.2 **Assert contact details never appear in a URL** — including on the PRG
      redirect, which is the place a later change would most plausibly leak them.

## 6. Accessibility — one bar, and it is modified

- [ ] 6.1 Every clause of the modified WCAG requirement asserted for the **service**
...
- [ ] 6.2 The catalogue as a grouped set with a `legend` naming what is being chosen.
- [ ] 6.3 Render each flow with no author stylesheet and confirm reading order and
      operability, as ⑤ did.

## 7. Spec hygiene

- [ ] 7.1 **Guarantee-diff the one MODIFIED requirement by hand.** Accessible,
...
- [ ] 7.3 The `default-frontend` spec's **Purpose** paragraph describes "a
      ...
      Update it at sync time.
- [ ] 7.4 Check whether `delivery-api` or `service-booking` say anything about the
      default front end that this change falsifies.
- [ ] 7.5 **One specific sibling, found at propose time and not yet resolved.**
...
- [ ] 8.1 Clean `dotnet build --no-incremental` with the TestSite stopped. **The
...
- [ ] 8.2 Unit, integration and client suites green, including every ⑤–⑨ scenario
      unchanged.
- [ ] 8.3 Live: book a multi-role service end to end with JavaScript disabled, and
      read the confirmation for both resources.
- [ ] 8.4 Live: provoke both refusals and read the two messages.
- [ ] 8.5 Live: the catalogue, and entry directly at a service.
- [ ] 8.6 **Launch the TestSite detached** (`Start-Process -WindowStyle Hidden`),
      never as a tracked background task; afterwards check port 44348 for orphans.

## 9. Handover

- [ ] 9.1 Record what ⑩-1 inherits and what it must add: the role-level flag deciding
      ...
      than defaulted.
- [ ] 9.2 Record whether the shared partials held, or started taking per-flow flags —
