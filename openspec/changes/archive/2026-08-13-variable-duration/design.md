...
- **After a rejected booking length, the length select displays the defaulted value while carrying the rejection message** — so it can read "too long" beside "30 minutes", and a screen reader announces as invalid a control whose current value is valid. Reachable when an editor narrows a resource's limits mid-session. → **Accepted, closed won't-fix (Chris, 2026-08-13)** on QA's assessment that it is MINOR: the spec requires only that the failure be reported, which it is, and the previous behaviour on this exact path was to silently confirm a booking nobody asked for. The two available fixes — naming the rejected length in the message, or rendering it as a `disabled selected` option — each trade one oddity for another. Revisit if the JS progressive-enhancement layer lands, which can repopulate the control without a round trip.
