...
- [x] 5.3 Verify the partial resolves when called from a **site** layout, not only from within the package (the mechanism is measured; this confirms this particular partial)
      → **Discharged by 7.3**, which needed the running TestSite: called from the dev harness's own `<head>`, the partial resolved and emitted `/_content/UBookIt.Web/ubookit.css`, which returned `200`. Additionally guarded by 7.9's two tests, which render it through the package's **own** rig — one that registers no tag helpers — so the `~/` resolution is proven on the package's terms rather than the TestSite's.
...
- [x] 9.7 Hand to `qa-review` in a **fresh context or subagent** — the context that implemented this must not review it
      → **Two rounds in a subagent that wrote none of the code.** Round 1 **REJECT** (1 CRITICAL, 2 MAJOR, 6 MINOR, 3 nits); round 2 **APPROVE WITH NITS**, no blockers, with the same agent resumed so it kept its round-1 findings. Both rounds are recorded in §10. The CRITICAL was a real accessibility defect that the whole guard set structurally could not see, and one of its own mutation checks certified — which is the strongest possible argument for the rule that the implementing context never reviews its own work.
