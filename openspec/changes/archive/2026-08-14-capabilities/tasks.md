...
- [x] 7.3 Live verification in the running backoffice, done against the pre-remediation build: tagged a resource, required that capability on a service, watched the readout change in both directions, and asserted every control association by reading the shadow DOM via `javascript_tool` (no dangling `aria-describedby`). This found and fixed the "1 resources" plural. Re-verified live after QA remediation (2026-08-14): the type-only wording renders ("3 resources have this type."); `aria-describedby` resolves with no dangling ids in BOTH states (hint alone, and error + hint together, the hint correctly retained); and the snapshot fix holds — sampling every 60ms through a capability removal, the readout never asserts the false "1 resource has this type", staying on the stale-but-true sentence until the new count lands.
...
- [x] 8.3 Sync-time sibling grep done, and it found three things beyond the delta-handled "eligibility by type key alone" sentence: the `services` Purpose still claimed duration semantics apply "when booking-via-service ships" (untrue since ⑦-2) and described a role as naming only a resource type; the `resources` Purpose omitted capabilities; and the `service-booking` Purpose was still the literal "TBD - created by archiving change book-via-service" placeholder ⑦-2 never replaced. All three corrected. Third consecutive change where this check found something.
