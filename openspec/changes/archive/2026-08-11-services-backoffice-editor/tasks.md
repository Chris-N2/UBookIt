...
- [x] 6.5 Run `qa-review` in a fresh context or subagent.
  ...
  - Second pass 2026-08-11: **APPROVE WITH NITS**, no must-fixes; 13/13 requirements traceable to code or test. Both remaining MINORs were faults in §8's own remediation and are fixed in §9. The review's one open caveat — that §8/§9 changed rendered markup which had been verified only by source reading — was then closed by the live shadow-DOM re-verification in §9.5, which confirmed the deduced behaviour in the running backoffice.
...
- [x] 9.5 **Live re-verification of the markup §8/§9 changed** — the gap the second review pass flagged as unclosed. Performed 2026-08-11 against the running TestSite by inspecting the editor's shadow DOM directly, not just by screenshot:
  - **Association, erroring group**: with only the duration guard active, the Duration `fieldset` carried `aria-describedby="err-duration"` resolving to a real element with the expected text, while Details and Requirements had **no attribute at all** (`hasAttribute` false, not `""`) and their ids were not rendered. With only a name failure active, the same held inverted: Details carried `err-details` with a valid referent, the other two nothing. Zero dangling ids in both states — confirming lit's `nothing` removes the attribute, as the review deduced from source.
  - **Radio group**: `role="radiogroup"`, accessible name "Duration mode", and the inert `label` attribute gone.
  - **Empty duration**: the Minutes field stays visibly empty instead of repainting `0`, and saving raises the announced `duration-required` message in the summary and the Duration group rather than silently submitting the previous 90. No request was issued.
  - **Focus**: still `div#error-summary[role="alert"]` after a failed save.
  - **Confirm modal after the §9.1 rewrite**: the modal opens and cancelling leaves the service listed with no console output and — the point of the check — **no spurious failure banner**, confirming a normal cancellation is not misclassified as a fault by the inverted discriminator.
