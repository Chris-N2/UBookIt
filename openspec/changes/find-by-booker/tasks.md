...
- [x] 4.3 Endpoint tests (the gate is a POLICY, so it is asserted against the attribute and the composed policy rather than by an in-process call that never meets it — see `EraseBookerEndpointTests` and `UBookItSensitiveDataAccessTests` for the composition and the handler): succeeds with the group; refused with section access alone; 401 anonymous; refusal is identical whether or not a match exists.
